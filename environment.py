...
HEIGHT_REQUIRED_PORTION = 0.5  # bottom 50%
WIDTH_REQUIRED_PORTION = 0.9

SHOW_PREVIEW = True

current_dir = os.path.dirname(__file__)
model_path = os.path.join(current_dir, "model_saved_from_CNN.h5")
...
class CarEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # discrete variable with 9 possible actions for steering
        ...
        # Example for using image as input normalised to 0 and 1
        ...
        self.model_3 = self.blueprint_library.filter("model3")[0] # Tesla Model 3 Car blueprint
    ...
    def maintain_speed(self, s):
        if s >= self.PREFERRED_SPEED:
            ...
            return 0.7  # full throttle
        else:
            return 0.3  # fine tune later
    ...
    def step(self, action):
        ...
        # use of pythagorean theorem to calculate velocity
    ...
    def reset(self):
        ...
        # apply random yaw so the RL does not guess to go straight
        ...
        self.steering_lock_start = None  # this is to count time in steering lock and punish for long time in steering lock
        self.step_counter = 0
        self.vehicle.apply_control(carla.VehicleControl(throttle=0.0, brake=0.0))
        self.image_for_CNN = self.apply_cnn(self.front_camera[self.height_from:, self.width_from:self.width_to])
    ...
    def process_img(self, image):
        ...
        i = i.reshape((self.im_height, self.im_width, 4))[:, :, :3]  # ignore the 4th Alpha channel of image, up to 3
        self.front_camera = i
